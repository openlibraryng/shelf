<!-- Developed by Gary Simcox -->

<template>
  <StackLayout ref="mainContainer" class="main-container" :visibility="visible?'visible':'collapse'">
    <Label class="main-label" text="OLI Shelf" :color="isLoggingIn? 'black' : 'white'"></Label>
  ...
  methods: {
    ...
    login() {
      if (getConnectionType() === connectionType.none) {
        alert("Shelf requires an internet connection to log in.")
        return;
      }
      ...
      return loginService
        ...
        .then(() => {
          ...
          console.log('navigated to shelf')
        })
        .catch((error) => {
          console.error(error)
    ...
    signUp() {
      if (getConnectionType() === connectionType.none) {
        alert("Shelf requires an internet connection to register.")
        return;
      }
    ...
    forgotPassword() {
      prompt({
        ...
        message: "Enter the email address you used to register for Shelf to reset your password.",
        defaultText: "",
        okButtonText: "Ok",
